...
//-----------------------------------------------------
	if (sdInfo.addr_mode == CHS_MODE)
	{
		...
		if (count == BLOCK_END)
		{
//			_10MB_unit = calc_unit(BOOT_PART_SIZE, sdInfo);
...
///////////////////////////////////////////////////////////
	block_start	= calc_unit(BOOT_PART_SIZE, sdInfo);
...
int get_mmc_block_count(char *device_name)
{
	...
	printf("mmc->capacity=%d mmc->read_bl_len=%d BLOCK_SIZE=%d\n", mmc->capacity, mmc->read_bl_len, BLOCK_SIZE);
