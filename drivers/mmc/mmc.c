...
static int mmc_read_ext_csd(struct mmc *host)
{
    ...
    if(host->capacity == 3825664)
    {
      if(ifwrite == 1)
      {
    		host->ext_csd.hs_max_dtr = 400000;
    		host->clock = 400000;
    	}
    	else
      {
...
int mmc_initialize(bd_t *bis)
{
	...
	if (mmc) {
		...
		if (err)
			err = mmc_init(mmc);
		if (err) {
			printf("SD/MMC0: Card init fail!\n");
//			return err;
		}
		else
		{
//#if defined(CONFIG_INAND_2G)
//    	printf("SD/MMC0: %ld MB\n", (mmc->capacity/(1024*1024/mmc->read_bl_len))/2);
//#else
...
//#endif
	...
	if (mmc) {
		...
		if (err)
			err = mmc_init(mmc);
		if (err) {	
			printf("SD/MMC2: Card init fail!\n");
			return err;
		}
		else
			printf("SD/MMC2: %ld MB\n", (mmc->capacity/(1024*1024/mmc->read_bl_len)));
	}
