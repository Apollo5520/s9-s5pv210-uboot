...
#define CONFIG_INAND_2G   1
//#define CONFIG_INAND_8G   1

#ifdef CONFIG_S5PV210_DEVBOARD
  #define CONFIG_HYNIX_MCP  1
#else
  #define CONFIG_DDR2       1
#endif

#define CONFIG_FASTBOOT		1
#define CONFIG_SDFUSE           /*通过组合按键从sd卡升级*/
//#define CONFIG_AUTO_SDFUSE  1     /*自动从sd卡升级*/
...
#define CFG_PROMPT              "SKDV210 # "   /* Monitor Command Prompt       */
...
//#define VPLL_VAL	(set_pll(VPLL_MDIV,VPLL_PDIV,VPLL_SDIV) | (0x1<<27))
...
#define CLK_DIV0_VAL    ((0<<APLL_RATIO)|(4<<A2M_RATIO)|(4<<HCLK_MSYS_RATIO)|(1<<PCLK_MSYS_RATIO)\
			|(3<<HCLK_DSYS_RATIO)|(1<<PCLK_DSYS_RATIO)|(4<<HCLK_PSYS_RATIO)|(1<<PCLK_PSYS_RATIO))
#elif defined(CONFIG_CLK_1000_166_166_133)
#define CLK_DIV0_VAL    ((0<<APLL_RATIO)|(4<<A2M_RATIO)|(5<<HCLK_MSYS_RATIO)|(1<<PCLK_MSYS_RATIO)\
			|(3<<HCLK_DSYS_RATIO)|(1<<PCLK_DSYS_RATIO)|(4<<HCLK_PSYS_RATIO)|(1<<PCLK_PSYS_RATIO))
#endif
...
//  #define DMC0_TIMING_DATA	0x23250304	// TimingData	CL=5
//  #define	DMC0_TIMING_PWR		0x09C80232	// TimingPower

  #define DMC0_TIMINGA_REF	0x00000618	// TimingAref	7.8us*133MHz=1038(0x40E), 100MHz=780(0x30C), 20MHz=156(0x9C), 10MHz=78(0x4E)
  #define DMC0_TIMING_ROW		0x1A233309	// TimingRow	for @200MHz
//  #define DMC0_TIMING_DATA	0x23230203	// TimingData	CL=3 RL=3 WL=2
  #define DMC0_TIMING_DATA	0x23250304	// TimingData	CL=5 RL=4 WL=3
  #define	DMC0_TIMING_PWR		0x09C80232	// TimingPower

  #define DMC0_PHYCONTROL1  0x00000086
...
//  #define DMC1_TIMING_DATA	0x23250304	// TimingData	CL=5
//  #define	DMC1_TIMING_PWR		0x08280232	// TimingPower

  #define DMC1_TIMINGA_REF	0x00000618	// TimingAref	7.8us*133MHz=1038(0x40E), 100MHz=780(0x30C), 20MHz=156(0x9C), 10MHz=78(0x4
  #define DMC1_TIMING_ROW		0x1A233309	// TimingRow	for @200MHz
//  #define DMC1_TIMING_DATA	0x23230203	// TimingData	CL=3
  #define DMC1_TIMING_DATA	0x23250304	// TimingData	CL=5 RL=4 WL=3
  #define	DMC1_TIMING_PWR		0x09C80232	// TimingPower
  //fighter--
#endif

#ifdef CONFIG_HYNIX_MCP
  #define	DMC0_MEMCONTROL		0x00212100
  #define DMC0_MEMCONFIG_0	0x20F01322	// MemConfig0	256MB config, 4 banks,Mapping Method[12:15]0:linear, 1:linterleaved, 2:Mixed
  #define DMC0_MEMCONFIG_1	0x30F01322	// MemConfig1
  #define DMC0_TIMINGA_REF	0x00000618	// TimingAref	7.8us*133MHz=1038(0x40E), 100MHz=780(0x30C), 20MHz=156(0x9C), 10MHz=78(0x4E)
  #define DMC0_TIMING_ROW		0x142342C8	// TimingRow	for @200MHz
  #define DMC0_TIMING_DATA	0x23230304	// TimingData	CL=3
  #define	DMC0_TIMING_PWR		0x0E1C0212	// TimingPower
  #define DMC0_PHYCONTROL1  0x00000085
#endif
...
#define CONFIG_NR_DRAM_BANKS    4          /* we have 4 bank of DRAM */
...
#define PHYS_SDRAM_3            (MEMORY_BASE_ADDRESS + 2 * SDRAM_BANK_SIZE) /* SDRAM Bank #3 */
#define PHYS_SDRAM_3_SIZE       SDRAM_BANK_SIZE
#define PHYS_SDRAM_4            (MEMORY_BASE_ADDRESS + 3 * SDRAM_BANK_SIZE) /* SDRAM Bank #4 */
...
#define CONFIG_IDENT_STRING	" for SKDV210"
...
#ifdef CONFIG_ENABLE_MMU
#define CFG_UBOOT_BASE		0xDFE00000
#else
#define CFG_UBOOT_BASE		0x3FE00000
#endif
...
/* IROM specific data */
#define SDMMC_BLK_SIZE        (0xD0037480)
#define COPY_SDMMC_TO_MEM     (0xD0037F98)
...
/* 
  分区信息：
  名称        大小      地址空间                  块空间 
  bootloader  (3M)      0x00000000 ~ 0x002FFFFF    0     ~ 6143 (6144)
  kernel      (5M)      0x00300000 ~ 0x005FFFFF    6144  ~ 16383(10240)
  ramdisk     (5M)      0x00800000 ~ 0x00CFFFFF    16384 ~ 26623(10240)
  logo        (37M)     0x00D00000 ~ 0x031FFFFF    26624 ~ 102399(75776)

  system      (250M)    0x03200000 ~ 0x12BFFFFF    102400  ~ 614399(512000)
  cache       (200M)    0x12C00000 ~ 0x1F3FFFFF    614400  ~ 1023999(409600)
  userdata    (1024M)   0x1F400000 ~ 0x7FFFFFFF    1024000 ~ 2199551(2097152)

  extern_sd   (ALL)     0x80000000 ~ END           2199552 ~
*/

#define MMC_BLOCK_SIZE        (0x200)

#define UBOOT_PART_START      (0x200)
#define UBOOT_PART_SIZE       (0x00300000-0x200)
#define UBOOT_PART_FILE       "bl_sekede.bin"
...
#define LOGO_PART_SIZE        (0x02500000)
#define LOGO_PART_FILE       "logo.bin"

#define SYSTEM_PART_FILE      "system.img"

/* pic location */
#define PIC_BOOT              LOGO_PART_START           //启动logo
#define PIC_UPDATE            (PIC_BOOT + 0x280000)     //升级界面

#define BOOT_PART_SIZE        (0x03200000)

#define SYSTEM_PART_SIZE      (0x0FA00000)
#define SYSTEM_PART_NUM       1
#define CACHE_PART_SIZE       (0x0C800000)
#define CACHE_PART_NUM        2
#define USER_DATA_PART_SIZE   (0x40000000)
#define USERDATA_PART_NUM     3
#define EXTSD_PART_NUM        4

//#define SEKEDE_PART_SIZE      (1024*1024*1024)
//#define SEKEDE_PART_NUM       2
////#define SEKEDE_PART_FAT32     1
//#define SEKEDE_PART_EXT2      1
...
/* Fastboot variables */
#define CFG_FASTBOOT_TRANSFER_BUFFER      (0x20000000)
#define CFG_FASTBOOT_TRANSFER_BUFFER_SIZE	(0x1FA00000)   /* 510 MB */
#define CFG_FASTBOOT_ADDR_KERNEL          (0x20008000)
#define CFG_FASTBOOT_ADDR_RAMDISK         (0x30A00000)
#define CFG_FASTBOOT_PAGESIZE             (2048)	// Page size of booting device
#define CFG_FASTBOOT_SDMMC_BLOCKSIZE      MMC_BLOCK_SIZE	// Block size of sdmmc
...
#define CONFIG_LCD_LG_V2      1
#define ANGLE_180						/*充电动画和图片显示旋转180度*/

#define CFG_LCD_FBUFFER       (0x3F000000)
#define CFG_LCD_TMP_BUFFER    (0x3E700000)
...
#define CONFIG_BOOTCOMMAND	"mmc read $boot_mmc 20008000 6144 10240 ; mmc read $boot_mmc 30A00000 16384 10240; bootm 20008000 30A00000"
...
#define CHECK_KEY_TIME  1  //10*100ms = 1S
#endif	/* __CONFIG_H */
