...
#define DBG_SETUP0(fmt, args...) Uart_printf(fmt)
#define DBG_SETUP1(fmt, args...) Uart_printf(fmt)
#define DBG_SETUP2(fmt, args...) Uart_printf(fmt)
#else
#define DBG_SETUP0(fmt, args...) ;
#define DBG_SETUP1(fmt, args...) ;
#define DBG_SETUP2(fmt, args...) ;
#endif
...
#else
#define DBG_BULK0(fmt, args...) ;
#define DBG_BULK1(fmt, args...) ;
#endif
...
#define FALSE	0
//Apollo +
#define SUSPEND_RESUME_ON TRUE
//#define SUSPEND_RESUME_ON FALSE
//Apollo -

unsigned int s3c_usbd_dn_addr = 0;
unsigned int s3c_usbd_dn_cnt = 0;
unsigned int remode_wakeup;
unsigned short config_value;
...
const unsigned char string_desc1[] = /* Manufacturer */
{
...
const unsigned char string_desc2[] = /* Product */
{
...
const unsigned char config_full[] =
{
	0x09,	/*  0 desc size */
...
const unsigned char config_full_total[] =
{
...
const unsigned char config_high[] =
{
	0x09,	/*  0 desc size */
	0x07,	/*  1 desc type (other speed)*/
	0x20,	/*  2 Total length of data returned */
...
const unsigned char config_high_total[] =
{
  0x09, 0x07 ,0x20 ,0x00 ,0x01 ,0x01 ,0x00 ,0xC0 ,0x19,
  0x09 ,0x04 ,0x00 ,0x00 ,0x02 ,0xff ,0x00 ,0x00 ,0x00,
...
unsigned char test_pkt [TEST_PKT_SIZE] = {
...
//	Outp32(S5P_OTG_GRSTCTL, CORE_SOFT_RESET);
...
void s3c_usb_wait_cable_insert(void)
{
	...
	do {
		...
		if (tmp & (B_SESSION_VALID|A_SESSION_VALID)) {
			DBG_SETUP0("OTG cable Connected!\n");
			break;
		} else if(ucFirst == 1) {
			DBG_SETUP0("Insert a OTG cable into the connector!\n");
//			ucFirst = 0;
		}
	} while(1);
}
...
void s3c_usb_check_current_mode(unsigned char *pucMode)
{
	unsigned int tmp;
...
int s3c_usbctl_init(void)
{
	...
	if (ucMode == INT_DEV_MODE) {
		...
		DBG_SETUP0("Error : Current Mode is Host\n");
		return 0;
	}
}
...
#ifdef USB_OTG_DEBUG_SETUP
void s3c_usb_print_pkt(unsigned char *pt, unsigned char count)
{
	int i;
...
void s3c_usb_verify_checksum(void)
{
	...
	cs_start = (unsigned char*)otg.dn_addr;
	cs_end = (unsigned char*)(otg.dn_addr+otg.dn_filesize-10);
	checkSum = 0;
	while(cs_start < cs_end) {
		checkSum += *cs_start++;
		if(((unsigned int)cs_start&0xfffff)==0) printf(".");
	}

	dnCS = (unsigned short)((cs_end[1]<<8) + cs_end[0]);
//	dnCS = *(unsigned short *)cs_end;
...
void s3c_usb_set_inep_xfersize(EP_TYPE type, unsigned int pktcnt, unsigned int xfersize)
{
...
void s3c_usb_set_outep_xfersize(EP_TYPE type, unsigned int pktcnt, unsigned int xfersize)
{
	if(type == EP_TYPE_CONTROL)
	{
...
void s3c_usb_write_ep0_fifo(unsigned char *buf, int num)
{
	int i;
...
void s3c_usb_write_in_fifo(unsigned char *buf, int num)
{
	int i;
	unsigned int data=0;
...
void s3c_usb_read_out_fifo(unsigned char *buf, int num)
{
	int i;
	unsigned int data;
	...
	for (i=0;i<num;i+=4)
	{
		...
		buf[i+1] = (unsigned char)(data>>8);
		buf[i+2] = (unsigned char)(data>>16);
		buf[i+3] = (unsigned char)(data>>24);
	}
}
...
void s3c_usb_get_desc(void)
{
	...
	case DEVICE_DESCRIPTOR:
		otg.req_length = (unsigned int)((otg.dev_req.wLength_H << 8) |
...
void s3c_usb_get_status(void)
{
	...
	case  (0x80):	/*device */
		DBG_SETUP1("DEVICE\n");
		get_status.Device=((unsigned char)remode_wakeup<<1)|0x1; /* SelfPowered */
...
#ifdef USB_OTG_DEBUG_SETUP
		s3c_usb_print_pkt((unsigned char *)&otg.dev_req, 8);
#endif
...
void s3c_usb_set_otherspeed_conf_desc(unsigned int length)
{
	...
	if (otg.speed == USB_HIGH)
	{
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 9);
			writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_full)+0, 9);
		}
	    else if(length ==32)
		{
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 32);
			writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_full_total)+0, 32);
	...
	else
	{
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_high)+0, 8);
		}
	    else if(length ==32)
		{
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_high_total)+0, 8);
	    }
...
void s3c_usb_transfer_ep0(void)
{
	...
	case EP0_STATE_GD_DEV_0:
		...
		if (otg.speed == USB_HIGH) {
			...
			if (otg.req_length < DEVICE_DESC_SIZE) {
				...
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.dev))+0, otg.req_length);
			} else {
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, DEVICE_DESC_SIZE);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.dev))+0, DEVICE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		} else {
			...
			if(otg.req_length<FS_CTRL_PKT_SIZE) {
				...
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.dev))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_DEV_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE)) {
			s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.dev))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_DEV_2:
		...
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		if(otg.req_length<DEVICE_DESC_SIZE) {
			s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.dev))+(2*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_CFG_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+0, otg.req_length);
			}
			else
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, CONFIG_DESC_TOTAL_SIZE);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+0, CONFIG_DESC_TOTAL_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_CFG_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE))
		{
			s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+FS_CTRL_PKT_SIZE,
						(otg.req_length-FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_2:
		...
		{
			s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+(2*FS_CTRL_PKT_SIZE),
						(otg.req_length-2*FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_3:
		...
		{
			s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+(3*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_DEV_QUALIFIER:	/*only supported in USB 2.0*/
		...
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
			s3c_usb_write_ep0_fifo((unsigned char *)qualifier_desc+0, otg.req_length);
		}
		else
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 10);
			s3c_usb_write_ep0_fifo((unsigned char *)qualifier_desc+0, 10);
		}
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_1:
		...
		if(otg.req_length==9)
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
				writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((unsigned char *)&config_high)+8, 1);
				otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
				writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((unsigned char *)&config_high)+8, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_2:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_high)+16, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_3:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo(((unsigned char *)&config_high)+24, 8);
			otg.ep0_state = EP0_STATE_INIT;
			break;
	...
	case EP0_STATE_GD_CFG_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			else
			{
				...
				writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.config))+0,
							CONFIG_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
	...
	case EP0_STATE_GD_IF_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			if(otg.req_length<INTERFACE_DESC_SIZE)
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.intf))+0, otg.req_length);
			}
			else
			{
				s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, INTERFACE_DESC_SIZE);
				writel(EPEN_CNAK_EP0_64, S5P_OTG_DIEPCTL0);
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.intf))+0, INTERFACE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.intf))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_IF_ONLY_1:
		...
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.intf))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_EP0_ONLY_0:
		...
		s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.ep1))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_EP1_ONLY_0:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, ENDPOINT_DESC_SIZE);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo(((unsigned char *)&(otg.desc.ep2))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_STR_I0:
		...
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)string_desc0, STRING_DESC0_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_STR_I1:
		...
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo((unsigned char *)string_desc1+(otg.ep0_substate*otg.ctrl_max_pktsize),
	...
	case EP0_STATE_GD_STR_I2:
		...
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
			s3c_usb_write_ep0_fifo((unsigned char *)string_desc2+(otg.ep0_substate*otg.ctrl_max_pktsize),
						otg.ctrl_max_pktsize);
			otg.ep0_state = EP0_STATE_GD_STR_I2;
			otg.ep0_substate++;
		} else {
	...
	case EP0_STATE_INTERFACE_GET:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_intf+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS0:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_status+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS1:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_status+1, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS2:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_status+2, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS3:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_status+3, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS4:
		...
		s3c_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, S5P_OTG_DIEPCTL0);
		s3c_usb_write_ep0_fifo((unsigned char *)&get_status+4, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
...
void s3c_usb_int_bulkin(void)
{
	...
	bulkin_buf = (unsigned char*)otg.up_ptr;
	remain_cnt = otg.up_size- ((unsigned int)otg.up_ptr - otg.up_addr);
...
void s3c_usb_upload_start(void)
{
	...
	s3c_usb_read_out_fifo((unsigned char *)tmp_buf, 10);
	check = *((unsigned char *)(tmp_buf+8)) + (*((unsigned char *)(tmp_buf+9))<<8);

	if (check==0x1) {
		otg.up_addr =
			*((unsigned char *)(tmp_buf+0))+
			(*((unsigned char *)(tmp_buf+1))<<8)+
			(*((unsigned char *)(tmp_buf+2))<<16)+
			(*((unsigned char *)(tmp_buf+3))<<24);

		otg.up_size =
			*((unsigned char *)(tmp_buf+4))+
			(*((unsigned char *)(tmp_buf+5))<<8)+
			(*((unsigned char *)(tmp_buf+6))<<16)+
			(*((unsigned char *)(tmp_buf+7))<<24);

		otg.up_ptr=(unsigned char *)otg.up_addr;
		...
		if (otg.op_mode == USB_CPU) {
			...
			unsigned int pktcnt, remainder;

			DBG_BULK1("Dma Start for IN PKT \n");

			writel(MODE_DMA|BURST_INCR4|GBL_INT_UNMASK,	S5P_OTG_GAHBCFG);
			writel(INT_RESUME|INT_OUT_EP|INT_IN_EP| INT_ENUMDONE|	INT_RESET|INT_SUSPEND, S5P_OTG_GINTMSK);

			writel((unsigned int)otg.up_ptr, S5P_OTG_DIEPDMA_IN);
      
      {
        ...
        pktcnt = 0;
        while((tmp - otg.bulkin_max_pktsize) > otg.bulkin_max_pktsize)
        {
          pktcnt ++;
          tmp = tmp - otg.bulkin_max_pktsize;
        }
        remainder = tmp;
//  			pktcnt = (unsigned int)(otg.up_size/otg.bulkin_max_pktsize);
//  			remainder = (unsigned int)(otg.up_size%otg.bulkin_max_pktsize);
		  }
			if(remainder != 0) {
				pktcnt += 1;
			}
...
void s3c_usb_download_start(unsigned int fifo_cnt_byte)
{
	unsigned char tmp_buf[8];

	s3c_usb_read_out_fifo((unsigned char *)tmp_buf, 8);
	...
	if (otg.op_mode == USB_CPU) {
		...
		unsigned int pkt_cnt, remain_cnt;
		...
		writel(MODE_DMA|BURST_INCR4|GBL_INT_UNMASK,
			S5P_OTG_GAHBCFG);
		writel((unsigned int)otg.dn_ptr, S5P_OTG_DOEPDMA_OUT);
      {
         unsigned int tmp;
         tmp = otg.dn_filesize-otg.bulkout_max_pktsize;
         
        pkt_cnt = 0;
        while((tmp - otg.bulkout_max_pktsize) > otg.bulkout_max_pktsize)
        {
          pkt_cnt ++;
          tmp = tmp - otg.bulkout_max_pktsize;
        }
        remain_cnt = tmp;
//		pkt_cnt = (unsigned int)(otg.dn_filesize-otg.bulkout_max_pktsize)/otg.bulkout_max_pktsize;
//		remain_cnt = (unsigned int)((otg.dn_filesize-otg.bulkout_max_pktsize)%otg.bulkout_max_pktsize);
		  }
		if(remain_cnt != 0) {
			pkt_cnt += 1;
		}
		...
		if (pkt_cnt > 1023) {
			s3c_usb_set_outep_xfersize(EP_TYPE_BULK, 1023, (otg.bulkout_max_pktsize*1023));
		} else {
			s3c_usb_set_outep_xfersize(EP_TYPE_BULK, pkt_cnt,	(otg.dn_filesize-otg.bulkout_max_pktsize));
		}

		/*ep3 enable, clear nak, bulk, usb active, next ep3, max pkt 64*/
		writel(1u<<31|1<<26|2<<18|1<<15|otg.bulkout_max_pktsize<<0, S5P_OTG_DOEPCTL_OUT);
	}
}

void s3c_usb_download_continue(unsigned int fifo_cnt_byte)
{
	if (otg.op_mode == USB_CPU) {
		s3c_usb_read_out_fifo((unsigned char *)otg.dn_ptr, fifo_cnt_byte);
		...
		if (((unsigned int)otg.dn_ptr - otg.dn_addr) >= (otg.dn_filesize - 8)) {
...
void s3c_usb_int_bulkout(unsigned int fifo_cnt_byte)
{
...
{
	signed int remain_cnt;

	DBG_BULK0("DMA IN : Transfer Done\n");

	otg.up_ptr = (unsigned char *)readl(S5P_OTG_DIEPDMA_IN);
	remain_cnt = otg.up_size- ((unsigned int)otg.up_ptr - otg.up_addr);
		...
		unsigned int pktcnt, remainder;
    {
       ...
       remainder = tmp;
//  		pktcnt = (unsigned int)(remain_cnt/otg.bulkin_max_pktsize);
//  		remainder = (unsigned int)(remain_cnt%otg.bulkin_max_pktsize);
    }
		if(remainder != 0) {
			pktcnt += 1;
		}
...
void s3c_usb_dma_out_done(void)
{
	...
	otg.dn_ptr = (unsigned char *)readl(S5P_OTG_DOEPDMA_OUT);

	remain_cnt = otg.dn_filesize - ((unsigned int)otg.dn_ptr - otg.dn_addr + 8);
		...
		unsigned int pktcnt, remainder;
    {
       ...
       remainder = tmp;
//  	  	pktcnt = (unsigned int)(remain_cnt/otg.bulkout_max_pktsize);
//    		remainder = (unsigned int)(remain_cnt%otg.bulkout_max_pktsize);
    }
		if(remainder != 0) {
			pktcnt += 1;
		}
		...
		if (pktcnt> 1023) {
			...
			s3c_usb_set_outep_xfersize(EP_TYPE_BULK, pktcnt, remain_cnt);
		}
...
/* dbg	  descConf.bConfigurationValue=2; // why 2? There's no reason.*/
	...
	otg.desc.ep1.wMaxPacketSizeL=(unsigned char)otg.bulkin_max_pktsize; /* 64*/
	otg.desc.ep1.wMaxPacketSizeH=(unsigned char)(otg.bulkin_max_pktsize>>8);
	...
	otg.desc.ep2.wMaxPacketSizeL=(unsigned char)otg.bulkout_max_pktsize; /* 64*/
	otg.desc.ep2.wMaxPacketSizeH=(unsigned char)(otg.bulkout_max_pktsize>>8);
...
int s3c_usb_set_init(void)
{
	...
	status = readl(S5P_OTG_DSTS); /* System status read */
//	while(!((((status&0x6) >>1) == USB_HIGH)|(((status&0x6) >>1) == USB_FULL)));
...
void s3c_udc_int_hndlr(void)
{
	...
	DBG_SETUP0("*** USB OTG Interrupt(S5P_OTG_GINTSTS)    ");
...
#define USBD_DOWN_ADDR 0x23e00000

unsigned int LoadFromUSB(void)
{
  DNW = 1;
  s3c_got_header = 0;
