...
//-----------------------------------------------------
	if (sdInfo.addr_mode == CHS_MODE)
	{
		...
		if (count == BLOCK_END)
		{
			_3MB_unit = calc_unit(_3MB, sdInfo);
//			partInfo->block_end	= sdInfo.C_end * sdInfo.H_end * sdInfo.S_end - _3MB_unit - 1;
			partInfo->block_end	= sdInfo.C_end * sdInfo.H_end * sdInfo.S_end - 1;
...
//-----------------------------------------------------
	else
	{
		...
		{
			_3MB_unit = calc_unit(_3MB, sdInfo);
			partInfo->block_end	= sdInfo.total_block_count - _3MB_unit - 1;
			partInfo->block_count	= partInfo->block_end - partInfo->block_start + 1;
...
///////////////////////////////////////////////////////////
// 반드시 Unit단위로 먼저 계산한다.
	block_start	= calc_unit(_3MB, sdInfo);
